...
//! Shared helpers for the machine crate's database-backed integration tests.
//!
//! Every test creates and drops its own scratch database, so a development database is never touched.
//! `QUANSIO_TEST_POSTGRES_URL` is the superuser DSN used to create the scratch database; when it is
//! unset the tests report `BLOCKED_EXTERNAL` and return, because the local dev stack is not running.
//!
//! This crate owns no other package's schema helpers: it sets the tenant context directly, the way
//! `quansio-events` and `quansio-indexer` do, so machine control has no dependency on the server crate.
...
/// Create an empty scratch database, apply the canonical migrations and return a pool bound to it.
...
/// Seed one tenant, user and workspace.
pub async fn seed_tenant(pool: &PgPool, tenant: &str, user: &str, workspace: &str) {
