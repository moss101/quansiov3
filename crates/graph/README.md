...
- `agent` — `agent_threads`/`agent_graph_edges`; delegation requires a visible parent in
  the same tenant/workspace, records the parent's delegation capability id, and calls a
  `DelegationNarrowingCheck` (RUN-005 plugs the Capability Projection algebra in).
- `runtime` — `runs`/`turns`/`steps`/`attempts`; the Run state machine, including
  `SUSPENDED`, and append-only attempts.
- `batch` — `apply_batch`: one transaction and one graph-revision compare-and-set across
  all three graphs; CORE-005 wraps it to add event emission.
